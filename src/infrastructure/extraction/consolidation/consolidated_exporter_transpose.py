...
def _extract_date_and_text(header_value: str) -> tuple:
    """
    Extract date code and header text from a column header.
    
    Examples:
        'Q3-2025 Carrying Value' -> ('Q3-2025', 'Carrying Value')
        'Q2-QTD-2023 Average Monthly Balance' -> ('Q2-QTD-2023', 'Average Monthly Balance')
        'Q3-YTD-2024' -> ('Q3-YTD-2024', '')
        'YTD-2024 Commitment' -> ('YTD-2024', 'Commitment')
        'Three Months Ended March 31, 2024' -> ('Q1-QTD-2024', '')
        
    Returns:
        Tuple of (date_code, header_text)
    """
    if not header_value or pd.isna(header_value):
        return ('', '')
    
    val_str = str(header_value).strip()
    
    # Pattern to match date codes: Q1-2024, Q2-QTD-2024, Q3-YTD-2024, YTD-2024
    date_pattern = r'^(Q[1-4](-QTD|-YTD)?-20\d{2}|YTD-20\d{2})'
    match = re.match(date_pattern, val_str)
    
    if match:
        # Header starts with a date code
        date_code = match.group(1)
        remaining = val_str[len(date_code):].strip()
        return (date_code, remaining)
    else:
        # Try to convert display format to code using QuarterDateMapper
        code = QuarterDateMapper.display_to_code(val_str)
        if code and code != val_str:
            return (code, '')
...
def create_transposed_dataframe(
    ...
    # Convert Dates column values to standard Y/Q format and extract header text
    # e.g., 'Q3-2025 Carrying Value' -> Dates: 'Q3-2025', Header Text: 'Carrying Value'
    dates_col = ('', 'Dates')
    
    # Find the integer location of the Dates column
    col_idx = -1
    ...
    # Extract date codes and header text from each row
    header_texts = []
    has_any_text = False
    ...
    if col_idx != -1:
        for row_idx in range(len(result_df)):
            ...
            if val is not None and pd.notna(val) and str(val).strip():
                ...
                date_code, header_text = _extract_date_and_text(val_str)
                
                # Update the Dates column with just the date code
                if date_code:
                    result_df.iloc[row_idx, col_idx] = date_code
                else:
    ...
    # Only add Header Text column if any text was found
    if has_any_text and header_texts:
        # Insert Header Text column after Dates column
        header_text_col = ('', 'Header Text')
        
        # Get current columns
        cols = list(result_df.columns)
        
        # Find position to insert (after Dates, before Source if present)
        insert_pos = col_idx + 1 if col_idx != -1 else 1
        
        # Create new column order with Header Text
        new_cols = cols[:insert_pos] + [header_text_col] + cols[insert_pos:]
        
        # Add the header text data
        result_df[header_text_col] = header_texts
        
        # Reorder columns
        result_df = result_df[new_cols]
        
        logger.debug(f"Added Header Text column with {sum(1 for t in header_texts if t)} non-empty values")
