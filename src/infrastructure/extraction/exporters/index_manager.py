...
class IndexManager:
    ...
    def update_index_for_split_sheets(cls, wb: Workbook, new_sheets: List[str]) -> None:
        ...
        # Find the source file from existing Index entries (should be like '10k1224.pdf')
        # Search for a valid PDF source filename in column A (Source)
        source_file = None
        for row_num in range(2, index_ws.max_row + 1):  # Skip header row
            cell_val = index_ws.cell(row=row_num, column=1).value
            if cell_val:
                val_str = str(cell_val).strip()
                # A valid source file should end with .pdf
                if val_str.lower().endswith('.pdf'):
                    source_file = val_str
                    break
        
        if not source_file:
            logger.warning("Could not find source file in Index, using placeholder")
            source_file = "unknown_source.pdf"
        ...
        for new_sheet_name in new_sheets:
            ...
            # Add to Index - properly use the source file, not the sheet name
            # Column A = Source (PDF filename), Column B = PageNo (unknown), 
            # Column C = Table_ID, Column F = Title
            index_ws.cell(row=next_row, column=1).value = source_file  # Source (PDF filename)
            index_ws.cell(row=next_row, column=2).value = ''  # PageNo (unknown for splits)
            index_ws.cell(row=next_row, column=3).value = new_sheet_name  # Table_ID
            index_ws.cell(row=next_row, column=4).value = new_sheet_name  # Location_ID
            index_ws.cell(row=next_row, column=5).value = ''  # Section (unknown)
            index_ws.cell(row=next_row, column=6).value = title  # Table Title
            
            # Create hyperlink in column G (Link)
            from openpyxl.worksheet.hyperlink import Hyperlink
            link_cell = index_ws.cell(row=next_row, column=7)
            link_cell.value = f"→ {new_sheet_name}"
            link_cell.hyperlink = Hyperlink(ref=link_cell.coordinate, target=f"#'{new_sheet_name}'!A1")
