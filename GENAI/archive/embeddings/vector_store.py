"""
Vector store with multi-provider support.

This module provides a backward-compatible interface while supporting
multiple VectorDB backends through the new provider system.
...
# Import ChromaDB implementation (default)
import chromadb
...
class VectorStore:
    ...
    For multi-provider support, use embeddings.vectordb_manager instead.
    """
    ...
    def __init__(
        ...
        print(f"Embedding provider: {self.embedding_manager.get_provider_info()['provider']}")
...
def get_vector_store(
    ...
    For multi-provider support, use:
        from embeddings.vectordb_manager import get_vectordb_manager
    
    Args:
        persist_directory: Directory to persist the database
        collection_name: Name of the collection
        provider: VectorDB provider (use vectordb_manager for this)
    """
    global _vector_store
    
    # If provider specified, use new system
    if provider is not None:
        from embeddings.vectordb_manager import get_vectordb_manager
        return get_vectordb_manager(provider=provider)
