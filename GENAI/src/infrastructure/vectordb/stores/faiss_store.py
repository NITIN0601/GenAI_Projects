...
class FAISSVectorStore(VectorStore):
    ...
    def search(
        ...
        for doc, score in docs_and_scores:
            try:
                # Ensure table_id exists (generate if missing for old data)
                meta_dict = dict(doc.metadata)
                if 'table_id' not in meta_dict or not meta_dict.get('table_id'):
                    # Generate table_id from source_doc and chunk_reference_id
                    source = meta_dict.get('source_doc', 'unknown')
                    chunk_ref = meta_dict.get('chunk_reference_id', str(uuid.uuid4())[:8])
                    meta_dict['table_id'] = f"{source}_{chunk_ref}"
                
                metadata = TableMetadata(**meta_dict)
            except Exception as e:
                # Fallback for very old or incomplete data
                logger.debug(f"Metadata validation failed: {e}")
                source_doc = doc.metadata.get("source_doc", "unknown")
                chunk_ref = doc.metadata.get("chunk_reference_id", str(uuid.uuid4())[:8])
                metadata = TableMetadata(
                    table_id=f"{source_doc}_{chunk_ref}",
