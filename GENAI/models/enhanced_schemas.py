"""Enhanced data models for advanced table structure extraction."""
...
class Period(BaseModel):
    """Standardized period information."""
...
class DocumentMetadata(BaseModel):
    ...
    file_hash: str = Field(..., description="MD5/SHA256 hash of file")
    company_name: Optional[str] = Field(None, description="Company name")
    document_type: Optional[str] = Field(None, description="Form 10-Q, 10-K, etc.")
...
class PageLayout(BaseModel):
    ...
    layout_type: str = Field(..., description="Layout type: single-column, two-column, multi-column, mixed")
    columns: List[Dict[str, float]] = Field(
        default_factory=list,
        description="Column boundaries with left and right coordinates"
    )
    gaps: List[float] = Field(default_factory=list, description="Vertical gap positions")
