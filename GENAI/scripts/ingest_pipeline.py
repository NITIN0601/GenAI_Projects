"""
Production PDF processing pipeline with unified extraction and vector DB storage.
Uses the new unified extraction system with automatic fallback and quality assessment.
"""
...
from extraction import UnifiedExtractor  # NEW: Unified extraction system
...
class ProductionPDFProcessor:
    ...
    def process_pdf(
        ...
        console.print(f"  [green][OK] Generated {len(embeddings)} embeddings[/green]")
        ...
        console.print(f"  [bold green][OK] Complete: {result['tables_extracted']} tables, {result['embeddings_generated']} embeddings[/bold green]")
    ...
    def _extract_with_docling(self, pdf_path: str):
        """Extract document using unified extraction system."""
        try:
            extractor = UnifiedExtractor(
                backends=["docling"],  # Use Docling backend
                enable_caching=True
            )
            result = extractor.extract(pdf_path)
            
            if not result.is_successful():
                console.print(f"  [red]✗ Extraction error: {result.error}[/red]")
                return None
            
            # Convert extraction result to document format
            # For now, return the result directly
            # TODO: Convert to EnhancedDocument format if needed
            return result
...
def main():
    ...
    if args.output:
        ...
        console.print(f"\n[green][OK] Results saved to {args.output}[/green]")
